...
class Fuse_Mk4(nn.Module):
    def __init__(self, input_size: int, output_size: int, fuse_method: int):
        ...
        self.fuse_method = fuse_method
        # Stage 1
        self.stage_1_A = Stage_Module(mod_type='conv_upsample', input_filters=input_size[-1], conv_filters=64)
        self.stage_1_B = copy.deepcopy(self.stage_1_A)
        self.stage_1_downsize = Stage_Module(mod_type='conv_downsize', conv_filters=64, pool_size=4)

        # Stage 2
        self.stage_2_A = Stage_Module(mod_type='conv_pool', conv_filters=64, pool_size=2)
        self.stage_2_B = copy.deepcopy(self.stage_2_A)
        self.stage_2_downsize = Stage_Module(mod_type='conv_downsize', conv_filters=64, pool_size=2)

        # Stage 3
        self.stage_3_A = Stage_Module(mod_type='conv_pool', conv_filters=64, pool_size=2)
        self.stage_3_B = copy.deepcopy(self.stage_3_A)

        # Stage 4
        self.stage_4_A = Stage_Module(conv_filters=64)
        self.stage_4_B = copy.deepcopy(self.stage_4_A)
